...
- Implement MSIP (software interrupt). This is really as simple as allowing a write to this bit and having it be an interrupt source.
- Implement mtime and mtimecmp
- Implement mcycle, minstret, and mcountinhibit, noting that they shouldn't increment on certain things like traps
- I need to check the manual again for the list of CSR's to see if I've missed anything (e.g. more timers)
...
- When an exception or trapping interrupt is detected (remember to check mstatus.MIE, mip and mie first), have a unit that decides the priority, (see table 17 from the manual), and then:
    ...
    - Have the ALU out of the writeback stage also be an input to the CSR file, so that you can set mtval
...
**Note**: one important question is how to deal with things like writes to special registers (e.g. mstatus.MIE). There may be enough special cases that the approach mentioned above wouldn't work.
...
- mtime: must increment at a constant frequency, and the platform must provide a mechanism for determining the period of an mtime tick (how?). It wraps around if it overflows. It is read-write. It is 64-bit, so on this architecture it has a high and low version.
- mtimecmp: also 64-bit. A machine timer interrupt becomes pending whenever mtime >= mtimecmp (unsigned). The interrupt remains posted until mtimecmp becomes greater than mtime (typically as a result of writing mtimecmp). The interrupt will only be taken if interrupts are enabled and the MTIE bit is set in the mie register
